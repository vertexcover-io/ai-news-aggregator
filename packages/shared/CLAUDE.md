...
- Exports `MODEL_PRICING` + `computeCallCost` and the provider-aware usage extractors in `pricing.ts` / `cost.ts`: `extractUsage(modelId, usage, providerMetadata)` is the dispatcher — model ids starting with `gemini-` route to `extractGeminiUsage` (reads only the standard AI SDK fields `inputTokens`/`outputTokens`/`cachedInputTokens`, forces the Anthropic-only `cacheCreation5mTokens`/`cacheCreation1hTokens` + `reasoningTokens` to 0), all other ids route to the unchanged `extractAnthropicUsage` (reads `providerMetadata.anthropic.usage.cache_creation` ephemeral tiers). Also exports the `RunCostBreakdown` / `CostStage` / `StageCost` / `ModelStageCost` types (in `types/cost-breakdown.ts`). The `run_archives.cost_breakdown` JSONB column is owned here; new pricing entries must include the five fields `inputPerMTok`, `outputPerMTok`, `cacheReadPerMTok`, `cacheWrite5mPerMTok`, `cacheWrite1hPerMTok` (no separate reasoning rate — thinking tokens bill at the output rate). `MODEL_PRICING` carries the Anthropic Claude entries plus a `gemini-3.1-flash-lite` entry (input $0.25, output $1.50, cache-read $0.025, cache-write tiers $0 per MTok) for the web-collector discovery/extraction stages.
