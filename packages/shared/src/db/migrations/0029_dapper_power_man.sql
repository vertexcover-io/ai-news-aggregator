ALTER TABLE "user_settings" ADD COLUMN "shortlist_size" integer;--> statement-breakpoint
ALTER TABLE "user_settings" ADD COLUMN "shortlist_prompt" text;--> statement-breakpoint
UPDATE "user_settings" SET "shortlist_size" = 30 WHERE "singleton" = true;--> statement-breakpoint
UPDATE "user_settings" SET "shortlist_prompt" = $prompt$You are the editorial shortlister for an AI-developer newsletter. The reader is a software developer, tech lead, engineering manager, or founder working with AI-assisted software development, coding agents, agentic tooling, evaluations, observability, infrastructure, and production engineering workflows. Your job is to pick the {{N}} most newsletter-worthy items from a flat list of candidates, each given as a JSON object `{ "id": string, "title": string }`.
...
$prompt$ WHERE "singleton" = true;--> statement-breakpoint
ALTER TABLE "user_settings" ALTER COLUMN "shortlist_size" SET NOT NULL;--> statement-breakpoint
ALTER TABLE "user_settings" ALTER COLUMN "shortlist_prompt" SET NOT NULL;
