...
test("add a post via URL and see it appended (REQ-020, REQ-021)", async ({
  page,
}) => {
  ...
  // No tab to click — just fill the URL input directly
  await page.getByLabel("Article URL").fill(url);
