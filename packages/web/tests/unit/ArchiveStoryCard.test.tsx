...
describe("ArchiveStoryCard", () => {
  ...
  it("left rail shows 12 for rank 12 (EDGE-009)", () => {
    render(<ArchiveStoryCard item={baseItem} rank={12} />);
  ...
  it("does not render LEAD STORY tag when rank === 2", () => {
    render(<ArchiveStoryCard item={baseItem} rank={2} />);
  ...
  it("eyebrow has no ▲ and no COMMENTS when both are zero (EDGE-007)", () => {
    ...
    const { container } = render(<ArchiveStoryCard item={item} rank={1} />);
  ...
  it("italic lede renders when recap.summary set (assert italic class present)", () => {
    const { container } = render(
      <ArchiveStoryCard item={itemWithRecap} rank={1} />,
    );
  ...
  // VER-94: right rail removed to dedupe rank (N°) and source label.
  it("does not render a right rail (rank/source dedup, VER-94)", () => {
    const { container } = render(<ArchiveStoryCard item={baseItem} rank={1} />);
    expect(container.querySelector('[data-rail="right"]')).toBeNull();
  });
  ...
  it("collapses to single-column layout on mobile with rank rail visible inline", () => {
    ...
    // Always-grid: one column on mobile, two on md+ (right rail removed in VER-94)
    expect(article.className).toContain("grid");
    expect(article.className).toContain("grid-cols-1");
    expect(article.className).toContain("md:grid-cols-[120px_minmax(0,1fr)]");
