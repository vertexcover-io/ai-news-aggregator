  ...
  return (
    <table data-testid="source-telemetry-table" className="w-full border-collapse">
      ...
      <tbody>
        {sources.map((source) => (
          <tr
            key={`${source.sourceType}:${source.identifier}`}
          ...
          >
            <td className="border-b border-line px-3 py-3 align-top font-mono text-[13px] text-ink-2">
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-[10px] uppercase tracking-[0.1em] text-mute-2">
                  {source.sourceType}
                </span>
              </div>
              {source.status === "failed" && source.errors.length > 0 ? (
                <div
                  ...
                  className="mt-1.5 max-w-[340px] text-[11.5px] leading-snug text-[#9d2f22]"
                >
                  {source.errors.join(" · ")}
                </div>
              ) : null}
