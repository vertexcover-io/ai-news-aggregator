...
- `src/api/` — typed API client (`client.ts` for the fetch wrapper, `runs.ts` incl. `getRunObservability(runId)` — 404 → null, `settings.ts`, `archives.ts`)
- `src/hooks/` — custom hooks (`useRunPolling.ts`, `useRunObservability.ts` — polls `GET /api/admin/runs/:runId/observability` every 2s while the run is non-terminal, stops on completed/failed/cancelled, treats 404 as null; mirrors `useRunPolling` without modifying it, and hooks for settings and review mutations)
