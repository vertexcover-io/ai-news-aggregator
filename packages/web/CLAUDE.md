...
- `src/hooks/` — custom hooks (`useRunPolling.ts`, `useRunObservability.ts` — polls `GET /api/admin/runs/:runId/observability` every 2s while the run is non-terminal, stops on completed/failed/cancelled, treats 404 as null; mirrors `useRunPolling` without modifying it; `useTriggerSocialPost(runId)` — wraps `triggerSocialPost` in a react-query mutation and invalidates the `["runs"]` query on success; `useReviewFilters(shortlistedItemIds)` — manages `shortlistOnly: boolean` + `selectedSources: string[]` (each entry is `"sourceType::identifier"`) filter state; exposes filtered ranked and pool lists; disables DnD when any filter is active; `useSourceFacets(runId)` — fetches `GET /api/admin/archives/:runId/source-facets` via react-query and returns `{sourceType, identifier, displayName, count}[]`; and hooks for settings and review mutations)

  Source filter — one option per COLLECTION UNIT (not per per-item identifier): the facet endpoint groups pool items by their stamped `metadata.sourceUnit` (`{identifier, displayName}`, written by the Reddit/Twitter collectors at collect time — the same identity Source Telemetry reports, e.g. `r/OpenAI`, `Twitter list 158…`, `@sama`). So a reddit/twitter LINK post groups under its subreddit / list rather than the linked article's domain, and a Twitter list is a single option rather than one-per-author. The dropdown shows `displayName`; the filter key is `identifier`. Items collected before this field existed carry no `sourceUnit` and therefore produce no source-filter options (old runs simply have no source filter — acceptable). The per-card badge still shows the URL-derived per-item identity (`@karpathy`, `404media.co`), which is independent of the unit-based filter.
