...
export function createRunLogRepo(
  ...
  return {
    async listForRun(runId: string): Promise<RunLogEntry[]> {
      ...
      const rows = await selectLogs(db, runId, scoped);
    ...
    async listForRunSource(
      ...
      if (!UUID_RE.test(runId)) return [];
      const exactRows = await selectLogs(db, runId, scoped, source.identifier);
      if (exactRows.length > 0) return exactRows.map(toRunLogEntry);

      if (source.identifier === source.sourceType) return [];
      const fallbackRows = await selectLogs(db, runId, scoped, source.sourceType);
...
function selectLogs(
  ...
  const predicates = [eq(runLogs.runId, runId)];
  if (!isAllTenants(scoped)) {
    predicates.push(eq(runLogs.tenantId, scoped.ctx.tenantId));
  }
  if (source !== undefined) {
    predicates.push(eq(runLogs.source, source));
  }
