...
export interface SourcesRepo {
  ...
  replaceAll(inputs: SourceCreateInput[]): Promise<SourceRecord[]>;
}
...
export function createSourcesRepo(
  ...
  return {
    ...
    // Bulk replace for the legacy settings write-through sync: PUT
    // /api/settings remains the source-list writer until the web Settings
    // panel migrates to /api/admin/sources (REQ-074). Replaces this
    // tenant's rows wholesale, so row ids and (not-yet-written) health are
    // reset on every settings save.
    async replaceAll(inputs: SourceCreateInput[]): Promise<SourceRecord[]> {
      return db.transaction(async (tx) => {
        await tx.delete(sources).where(eq(sources.tenantId, tenantId));
