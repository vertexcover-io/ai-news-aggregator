...
describe("PATCH /api/admin/archives/:runId — immediate publish block", () => {
  describe("REQ-002, EDGE-008: all channels enabled + past-due → all three enqueued", () => {
    it("enqueues email-send, linkedin-post, twitter-post with correct args", async () => {
      vi.setSystemTime(NOW_PAST_DUE);

      const updatedRow = makeArchiveRow({ reviewed: true });
  ...
  describe("REQ-003, EDGE-007: email past-due, linkedin future → only email-send enqueued", () => {
    it("enqueues only email-send when linkedin and twitter times are in the future", async () => {
      // now = 07:30 UTC on 2026-01-15, so email (07:00) is past-due
      // but linkedin (08:00) and twitter (09:00) are still in the future
      const nowBetweenEmailAndLinkedin = new Date("2026-01-15T07:30:00Z");
      vi.setSystemTime(nowBetweenEmailAndLinkedin);
  ...
  describe("REQ-008: channel with non-null sent timestamp → not enqueued", () => {
    it("skips email-send when emailSentAt is set even if past-due", async () => {
      vi.setSystemTime(NOW_PAST_DUE);

      const updatedRow = makeArchiveRow({
        reviewed: true,
        emailSentAt: new Date("2026-01-15T07:05:00Z"), // already sent
      });
      const { queue, addSpy } = makeQueue();
      const { app } = buildApp({
        archiveRow: makeArchiveRow({ reviewed: false }),
      ...
      expect(res.status).toBe(200);
      // email-send should be skipped; linkedin and twitter still enqueued
      expect(addSpy).not.toHaveBeenCalledWith(
        "email-send",
        expect.anything(),
        expect.anything(),
    ...
    it("skips linkedin-post when linkedinPostedAt is set even if past-due", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
    ...
    it("skips twitter-post when twitterPostedAt is set even if past-due", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
      ...
      expect(addSpy).toHaveBeenCalledWith("email-send", expect.anything(), expect.anything());
      expect(addSpy).toHaveBeenCalledWith("linkedin-post", expect.anything(), expect.anything());

      vi.useRealTimers();
    });
  });

  describe("REQ-010: processingQueue undefined → no enqueue, handler still returns 200", () => {
    it("no processingQueue dep → PATCH returns 200 with updated archive, no enqueue", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
    ...
    it("getSettingsRepo undefined → PATCH returns 200, no enqueue", async () => {
      vi.setSystemTime(NOW_PAST_DUE);

      const updatedRow = makeArchiveRow({ reviewed: true });
      const { queue, addSpy } = makeQueue();
      // No getSettingsRepo, but processingQueue is present
      const archiveRepo = makeArchiveRepo(makeArchiveRow({ reviewed: false }), updatedRow);
      const app = new Hono();
      app.route(
        "/api/admin/archives",
    ...
    it("settings is null → PATCH returns 200, no enqueue", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
  ...
  describe("REQ-004 via helper: disabled channel past-due → not enqueued", () => {
    it("twitterPostEnabled=false → twitter-post not enqueued even when past-due", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
      ...
      const { app } = buildApp({
        archiveRow: makeArchiveRow({ reviewed: false }),
        updatedRow,
        settings: makeSettings({ twitterPostEnabled: false }),
        processingQueue: queue,
      });
      ...
      // email-send and linkedin-post still enqueued
      expect(addSpy).toHaveBeenCalledWith("email-send", expect.anything(), expect.anything());
      expect(addSpy).toHaveBeenCalledWith("linkedin-post", expect.anything(), expect.anything());
    ...
    it("scheduleEnabled=false → no channels enqueued", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
      ...
      const { app } = buildApp({
        archiveRow: makeArchiveRow({ reviewed: false }),
        updatedRow,
        settings: makeSettings({ scheduleEnabled: false }),
        processingQueue: queue,
      });
  ...
  describe("REQ-009: emits archive.immediate_publish_enqueued log", () => {
    it("logs info with event and enqueued channels after past-due enqueue", async () => {
      vi.setSystemTime(NOW_PAST_DUE);
      ...
      // Find the immediate-publish log call
      const publishLog = infoSpy.mock.calls.find(
        (call: unknown[]) =>
          typeof call[0] === "object" &&
          call[0] !== null &&
          (call[0] as Record<string, unknown>).event === "archive.immediate_publish_enqueued",
      );
  ...
  describe("EDGE-003: autoReview value in settings does NOT affect immediate block", () => {
    it("autoReview=true in settings does not suppress the immediate block on manual PATCH", async () => {
      vi.setSystemTime(NOW_PAST_DUE);

      const updatedRow = makeArchiveRow({ reviewed: true });
      const { queue, addSpy } = makeQueue();

      // autoReview=true in settings — should NOT suppress the immediate block
      const settingsWithAutoReview = makeSettings({ scheduleEnabled: true });
      settingsWithAutoReview.autoReview = true;

      const archiveRepo2 = makeArchiveRepo(
      ...
      const res = await app2.request("/api/admin/archives/run-uuid-1", {
        method: "PATCH",
        headers: PATCH_HEADERS,
        body: PATCH_BODY,
      });
      ...
      expect(res.status).toBe(200);
      // autoReview=true doesn't suppress — all three channels still enqueued
      expect(addSpy).toHaveBeenCalledTimes(3);
