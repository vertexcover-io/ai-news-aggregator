...
function makeMinimalDeps(): BuildAppDeps {
  const stub = () => new Hono();
  const requireUserFactory = (_secret: string): MiddlewareHandler =>
    async (_c, next) => {
      await next();
    };
  return {
    ...
    publicTenantConfigRouter: stub(),
    publicTenantLogoRouter: stub(),
    adminBrandingRouter: stub(),
  };
}
