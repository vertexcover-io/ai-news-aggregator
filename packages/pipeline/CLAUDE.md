...
## Layout
- `src/collectors/` — one file per source (`hn.ts`, `reddit.ts`, `web.ts`); each exports both the batch collector (`collectHn`, `collectReddit`, `collectWeb`) called by the run-process worker AND the single-post fetcher (`fetchHnPost`, `fetchRedditPost`, `fetchWebPost`) used by the add-post flow. `web.ts` also exports `extractFallbackImage` for OG/twitter/favicon image extraction; its private parsing helpers live in `web-image-fallback.ts`.
- `src/lib/` — shared low-level utilities: `fetch-with-retry.ts` (generic retry wrapper), `date-filter.ts` (shared `filterBySinceDays` helper used by collectors), `abortable-fetch.ts` (wraps `fetch` to honour a run's `AbortSignal`), `cancelled-error.ts` (`CancelledError` class for run-cancellation signalling).
...
- `src/services/` — `run-state.ts` (Redis-backed per-run status read/write), `candidate-loader.ts` (loads `raw_items` rows for ranking), `recency.ts` (half-life decay helper), `markdown-fetch.ts` (shared Jina-backed `fetchMarkdown({ signal?: AbortSignal })`), and `add-post-helper.ts` (`hydrateAddedPost()` orchestrates single-item fetch + recap; `dispatchFetch()` forwards both `signal` and `fetchFn` to the appropriate single-post collector)
