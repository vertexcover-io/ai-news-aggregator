...
- The `run-process` worker is the single BullMQ job per run — it runs all requested collectors concurrently in-process, then passes items through a 7-stage pipeline: noise filter → recency gravity → semantic dedup → shortlist → rank → MMR selection
...
- `src/processors/` — pure stage functions (`dedup.ts`, `noise.ts`, `semantic-dedup.ts`, `shortlist.ts`, `rank.ts`, `mmr.ts`); `noise.ts` drops low-signal items by title pattern and engagement threshold; `semantic-dedup.ts` clusters by cosine similarity >0.85 on title embeddings and merges engagement across sources; `shortlist.ts` is the Voyage-embedding cosine shortlist (accepts pre-computed titleEmbeds, returns them for downstream use); `rank.ts` is the Claude Haiku reranker using Vercel AI SDK `generateObject` with 1-5 per-axis scoring, a quality gate, and 4-signal fusion, with prompts inlined as TS consts in `rank-prompts.ts`; `mmr.ts` applies Maximal Marginal Relevance (λ=0.7) with a 3-per-sourceType cap
...
- `src/services/` — `run-state.ts` (Redis-backed per-run status read/write), `candidate-loader.ts` (loads `raw_items` rows for ranking; the `Candidate` type lives in `@newsletter/shared/types`), `embeddings.ts` (Voyage AI client for embeddings), `recency.ts` (recency helpers: `recencyGravity(ageHours)` = `1/(ageHours+2)^1.5` and `ageHoursFromPublishedAt`), and `markdown-fetch.ts` (shared Jina-backed `fetchMarkdown({ signal?: AbortSignal })`, used by the web collector and rank body loader)
...
- `ANTHROPIC_API_KEY` and `VOYAGE_API_KEY` are both validated at worker startup (not per job) — the rank stage always needs Anthropic, semantic dedup and shortlist always need Voyage. `RANKING_MODEL` defaults to `claude-haiku-4-5-20251001`.
