...
function makeFakeDb(initial: StoredRow[] = []): FakeDbHandle {
  ...
  // The repo now composes and(eq(tenantId), eq(platform)) — the platform Param
  // sits inside a nested SQL chunk, so walk the predicate tree recursively and
  // pick out the value that looks like a platform (ignoring the tenant UUID).
  const PLATFORMS = new Set(["linkedin", "twitter", "twitter_collector"]);
  function findPlatform(node: unknown): string | null {
    if (typeof node === "string") return PLATFORMS.has(node) ? node : null;
    if (node === null || typeof node !== "object") return null;
    if ("value" in node) {
      const v = (node as { value: unknown }).value;
      if (typeof v === "string" && PLATFORMS.has(v)) return v;
    }
...
describe("SocialTokensRepo (cipher-aware) -- encrypt->save->read round-trip (REQ-006)", () => {
  it("getToken returns the original plaintext access and refresh tokens after saveToken", async () => {
    ...
    const repo = createSocialTokensRepo(handle.db, TENANT_ZERO_ID, cipherA());
  ...
  it("withTokenLock callback receives decrypted accessToken and refreshToken", async () => {
    ...
    const repo2 = createSocialTokensRepo(handle.db, TENANT_ZERO_ID, cipherA());
...
describe("SocialTokensRepo (cipher-aware) -- wrong SESSION_SECRET returns null on decrypt (EDGE-005)", () => {
  it("getToken returns null when trying to decrypt with a different SESSION_SECRET", async () => {
    ...
    const repoA = createSocialTokensRepo(handle.db, TENANT_ZERO_ID, cipherA());
    ...
    const repoB = createSocialTokensRepo(handle.db, TENANT_ZERO_ID, cipherB());
