...
Required for a full run: `DATABASE_URL`, `REDIS_URL`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, `SESSION_SECRET` (≥32 bytes; doubles as HKDF KEK for credential encryption at rest — never rotate without a re-connect plan, EC10).

Multi-tenant host resolution: `APP_ROOT_DOMAIN` (default `lvh.me`; tenants live at `<slug>.<root>`), `APP_HOST` (default `app.<root>`; admin surface, no public tenant routes), `TENANT0_CUSTOM_DOMAIN` (optional legacy apex → tenant 0). In non-prod an `X-Tenant-Slug` header overrides Host. Migration/seed scripts read `AGENTLOOP_ADMIN_EMAIL`/`AGENTLOOP_ADMIN_PASSWORD` and `SUPER_ADMIN_EMAILS`/`SUPER_ADMIN_PASSWORD`. Full reference + dev workflow: `docs/multi-tenant/dev.md`; proxy/TLS wiring: `docs/multi-tenant/deploy.md`.

Optional integrations (each disabled when unset): `SLACK_WEBHOOK_URL`, `PUBLIC_BASE_URL`, `TAVILY_API_KEY` (web search + onboarding source discovery), `RETTIWT_API_KEY`, LinkedIn OAuth app (`LINKEDIN_CLIENT_ID/SECRET/API_VERSION`), Twitter OAuth 2.0 app for per-tenant connect (`TWITTER_OAUTH_CLIENT_ID/SECRET`), legacy Twitter OAuth 1.0a (`TWITTER_API_KEY/SECRET/ACCESS_TOKEN/ACCESS_TOKEN_SECRET`), email (`EMAIL_PROVIDER` resend|ses, `RESEND_API_KEY` — full-access key required for sending-domain register/verify, `AWS_REGION`/`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`, `FROM_MAIL`, `BROADCAST_FROM_LOCALPART`, `NEWSLETTER_REPLY_TO_EMAIL`), `RANKING_MODEL`, `SHORTLIST_MODEL`, `WEB_CRAWLER_CONCURRENCY`, `EMAIL_SEND_RATE_PER_SECOND`, `PIPELINE_RUN_CONCURRENCY` (run cap, default 1; read only by the dedicated run-process worker — inert in the deployed dispatcher topology, where the cap is BullMQ's default 1), `PIPELINE_START_JITTER_MS` (scheduled-run start jitter window, default 180000; 0 disables), `TWITTER_COLLECTOR_RATE_PER_SECOND` (cross-tenant rettiwt fetch throttle, default 1; ≤0 disables), PostHog analytics + error tracking (`POSTHOG_PROJECT_TOKEN` or `POSTHOG_API_KEY`, `POSTHOG_HOST`, `POSTHOG_ENABLED`). `TRUST_PROXY_HOPS` (count of trusted reverse proxies; auth rate limiting only honors `X-Forwarded-For` when set ≥1 — deploys behind a proxy MUST set it or per-IP limits key on the proxy address, and without a proxy it MUST stay 0 or the header is client-spoofable).
