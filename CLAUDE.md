...
**Key data flow (current — personalized two-stage ranking with recap):** User picks a profile (from `profiles/*.yaml` at the repo root, exposed via `GET /api/profiles`) and submits HN + Reddit config on `/run`, optionally with `profileName` and `halfLifeHours` in `POST /api/runs` -> API enqueues a single job on the processing queue via `Queue.add` with `jobId: runId` -> The `run-process` worker runs all collectors concurrently in-process (via `Promise.allSettled`), writing to `raw_items` (including `imageUrl` extracted per source: Reddit preview/thumbnail, web collector LLM-selected image, HN OG image) -> dedups -> **stage-1 shortlist** embeds the profile + candidates via Voyage AI (`VOYAGE_API_KEY`) and selects top-K by cosine similarity with recency decay -> **stage-2 rerank** feeds the shortlist (with body + comments) to Claude Haiku via Vercel AI SDK to produce the final ordering with rationale **plus structured recap content** (`summary`, `bullets`, `bottomLine`) per item -> writes `rankedItems` (including recap fields) to Redis run-state and recap content to `raw_items.metadata.recap` -> Frontend polls `GET /api/runs/:runId` and renders the ranked list -> After the run completes, a "View Archive" button navigates to `/archive/:runId`, a recap-style archive page that renders each `RankedItem` as a story card with image, "The Recap:" summary, "Unpacked:" bullet analysis, "Bottom line:" takeaway, and "Read more" link.
