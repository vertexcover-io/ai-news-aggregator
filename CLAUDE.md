...
Required env vars for a full run: `DATABASE_URL`, `REDIS_URL`, `ANTHROPIC_API_KEY`, `JINA_API_KEY`, `ADMIN_PASSWORD` (shared password for `/admin/*`), `SESSION_SECRET` (HMAC secret for admin session cookie); optional `RANKING_MODEL`.
