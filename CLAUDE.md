...
Required env vars for a full run: `DATABASE_URL`, `REDIS_URL`, `ANTHROPIC_API_KEY`, `ADMIN_PASSWORD` (shared password for `/admin/*`), `SESSION_SECRET` (HMAC secret for admin session cookie); optional `RANKING_MODEL`, `WEB_CRAWLER_CONCURRENCY` (default 4).
