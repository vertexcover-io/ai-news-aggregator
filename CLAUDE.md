...
**Key data flow (current — 7-stage ranking pipeline):** User picks a profile (from `profiles/*.yaml` at the repo root, exposed via `GET /api/profiles`) and submits HN + Reddit config on `/run`, optionally with `profileName` in `POST /api/runs` -> API enqueues a single job on the processing queue via `Queue.add` with `jobId: runId` -> The `run-process` worker runs all collectors concurrently in-process (via `Promise.allSettled`), writing to `raw_items` -> **stage-1 noise filter** drops titles matching noise patterns (Ask HN:, Who is hiring?, etc.) and items below engagement thresholds (HN<5, Reddit<10) -> **stage-2 recency gravity** scores items with `1/(ageHours+2)^1.5` -> **stage-3 semantic dedup** clusters items by cosine similarity >0.85 on title embeddings (Voyage AI), merging engagement across sources -> **stage-4 shortlist** embeds the profile + candidates via Voyage AI and selects top-K by cosine similarity (no recency at this stage), passing pre-computed embeddings forward -> **stage-5 rank** feeds the shortlist (with body + comments) to Claude Haiku via Vercel AI SDK for 1-5 per-axis scoring (relevance/novelty/signalVsHype/actionability) with a quality gate (mean<2.0 drops) and 4-signal fusion (0.40×llm + 0.25×engagement + 0.20×recency + 0.15×authority with profile; 0.50×llm + 0.30×engagement + 0.20×recency without profile) -> **stage-6 MMR selection** applies Maximal Marginal Relevance (λ=0.7) with a 3-per-sourceType cap to diversify results -> writes `rankedItems` to Redis run-state -> Frontend polls `GET /api/runs/:runId` and renders the ranked list.
