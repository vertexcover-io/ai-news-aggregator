...
Required env vars for a full run: `DATABASE_URL`, `REDIS_URL`, `ANTHROPIC_API_KEY`, `ADMIN_PASSWORD` (shared password for `/admin/*`), `SESSION_SECRET` (HMAC secret for admin session cookie AND HKDF KEK for the `social_credentials` AES-256-GCM cipher; must be ≥32 bytes); optional `RANKING_MODEL`, `WEB_CRAWLER_CONCURRENCY` (default 4), `SLACK_WEBHOOK_URL` (Slack notification on review-completion; disabled when unset), `PUBLIC_BASE_URL` (used in Slack message archive links), `LINKEDIN_CLIENT_ID` / `LINKEDIN_CLIENT_SECRET` / `LINKEDIN_API_VERSION` (LinkedIn auto-post; disabled when unset), `TWITTER_API_KEY` / `TWITTER_API_SECRET` / `TWITTER_ACCESS_TOKEN` / `TWITTER_ACCESS_TOKEN_SECRET` (X/Twitter OAuth 1.0a auto-post; disabled when unset). The LinkedIn and Twitter credential sets above may also be managed at `/admin/settings` — values are stored encrypted at rest in the `social_credentials` table and resolved per pipeline job by a DB-first / env-fallback resolver, so saving credentials via the admin UI takes effect on the next pipeline job without a worker restart and shadows any env-var values.
