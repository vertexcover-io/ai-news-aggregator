...
| REQ-003 | Event-driven | When the page mounts with a completed run, it shall render a mono hero eyebrow matching `<WEEKDAY_UPPERCASE> · <MONTH DD, YYYY> · ISSUE N°<issueNumber>`. | DOM contains the exact format; tested with fixed clock. | Must | **Partially implemented — API gap:** The `ISSUE N°<issueNumber>` segment is omitted from the rendered eyebrow because the `GET /api/archives/:runId` response does not expose an issue number field. The eyebrow renders `<WEEKDAY> · <MONTH DD, YYYY>` only. Follow-up work: add `issueNumber` to the `RunDetail` API response shape and thread it through `ArchivePageHeader`. See REVIEW-1.md I-1/I-2. |
| REQ-004 | Event-driven | When the run has a non-empty `leadSummary`, the page shall render `leadSummary` as the hero serif headline; otherwise it shall render the top-ranked story's title. | Two unit tests (with/without leadSummary) assert headline text. | Must | **Partially implemented — API gap:** `leadSummary` is hardcoded to `null` in `ArchivePage.tsx` because the `GET /api/archives/:runId` response does not include a `leadSummary` field (it lives on listing-level data only). The page always falls back to the top story's title as the headline. Follow-up work: expose `leadSummary` in the archive detail API response. See REVIEW-1.md I-1. |
...
| REQ-028 | Ubiquitous | The page shall set `document.title` to `Issue N°<issueNumber> — <runDate>` and a meta description to the hero headline. | `document.title` and `<meta name="description">` assertions pass after mount. | Should | **Partially implemented — API gap:** `document.title` is set to `Issue — <runDate>` (no `N°<issueNumber>` prefix) because issue number is not available in the API response. Follow-up work: same as REQ-003 — add `issueNumber` to the detail endpoint. See REVIEW-1.md I-2 and quality-gate finding 2. |
