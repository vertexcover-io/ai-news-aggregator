...
**Pattern to follow:** `packages/pipeline/src/lib/fetch-with-retry.ts` (`fetchWithRetry`). Import and use directly — the shared utility handles retries, exponential backoff via `Math.pow(2, attempt) * 1000`, and short-circuit on non-retryable 4xx.
