...
**Source:** `docs/multi-tenant/design.md`
...
> Visual reference for every surface below: the HTML mockups in `docs/multi-tenant/mocks/` (`index.html`). VS-1 → `signup.html` + `onboarding.html`; VS-3 → `public-home.html`; VS-4 → `super-admin.html` + `impersonation.html`; VS-5 → `settings.html` (Sending domain panel).
