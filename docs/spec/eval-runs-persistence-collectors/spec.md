...
| [`03-eval-fixture-new.html`](../../mocks/eval-redesign/03-eval-fixture-new.html) | REQ-7 / REQ-8 (collector resolution) — the design notes already say the chip preview is gone; users only see the result after submit. The post-submit navigation target is `/admin/eval/grade/<id>` (operator grades, then Save-to-repo lands them on `/admin/eval?fixtureId=<id>`) — handled in Stage C. |
...
### Flow A: "Build a fixture, grade it, then eval"
...
4. UI navigates to `/admin/eval/grade/<new-id>` — operator labels each cluster as must / nice / drop.
5. Operator clicks **Save to repo**. Ground-truth file is persisted; UI navigates to `/admin/eval?fixtureId=<new-id>` with the fixture pre-selected in Mode A picker (commit `d37ecb3`).
6. User clicks **Run scored eval**. SSE stream starts.
7. At stream start, a row INSERTs in `eval_runs` (REQ-2). User sees per-fixture rows stream in (sessionStorage persists them — implemented in `d37ecb3`).
8. Stream ends. Row UPDATEs to `done` (REQ-3). UI shows the aggregate hero strip with real nDCG@10 because the fixture has ground truth.

**Why grade-first:** an ungraded fixture has no labels, so nDCG@10 has no signal to compute against. Skipping grading would land the operator on the eval page only to get `score: null`. The two-hop flow (`build → grade → eval`) ensures every eval run produces meaningful scores.
...
| REQ-12 | Event-driven | When `createManualFixture` finishes and the API route returns, the response shape shall let the UI navigate to `/admin/eval/grade/<new-id>` (the grade-first Flow A target). | The existing POST `/api/admin/eval/fixtures` response already returns `{ fixtureId, itemCount }` — no change required server-side. The navigation choice is purely UI (Stage C). This REQ exists to record that the contract is intentionally unchanged. | Must |
