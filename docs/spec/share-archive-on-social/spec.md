...
| REQ-009 | Event-driven | When both Clipboard API and `execCommand` fail, the system shall set the button label to `"COPY FAILED"` for 1500 ms and emit a single `console.warn` describing the failure. | Component test stubs both as failing; asserts label = `"COPY FAILED"`; asserts `console.warn` called once. | Should |
