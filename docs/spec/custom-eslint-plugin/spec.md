...
| REQ-084 | Unwanted | If any file under `packages/**`, `tools/**`, or `docs/**` contains the strings `docker-compose` or `docker ` (outside of explicit allowlist comments), then the script shall print the offending file+line and exit with code 1. | Fixture: add a `docker-compose up` line in a script; exit code is 1. | Should |
