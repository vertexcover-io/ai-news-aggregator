...
| `/admin/eval/fixtures/new` (EvalManualFixturePage) | `03-eval-fixture-new.html` | Action bar moves below the textarea + invalid-lines panel; pipeline explainer + source-mix preview rail; navigates to `/admin/eval/grade/<id>` on success (operator grades the fixture before running scored evals against it). |
...
| REQ-8 | Event-driven | When `EvalManualFixturePage` successfully creates a fixture, the system shall navigate to `/admin/eval/grade/<new-id>` so the operator grades the fixture before running scored evals against it. After Save-to-repo completes on the grade page, the system shall navigate to `/admin/eval?fixtureId=<new-id>` with the fixture pre-selected in Mode A's Single-fixture picker. | Two-hop flow: build → grade → eval. The grade-page's Save-to-repo redirect (commit `d37ecb3`) closes the loop. | Must |
...
| REQ-8 | Yes | No | No | Yes | Manual-fixture unit test asserts the post-submit destination is `/admin/eval/grade/:fixtureId`. Save-to-repo round-trip back to `/admin/eval?fixtureId=` is covered by the existing EvalGradePage unit test from commit `d37ecb3`. |
