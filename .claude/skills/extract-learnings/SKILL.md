...
Do NOT wire the stub into `eslint.config.mjs` as an active rule — leave that for a human reviewer to promote once the implementation lands. The stub exists only so the next maintainer has a starting point and a breadcrumb back to the learning.
