...
- Collector health (index.ts): HEALTH_CHECKABLE_COLLECTORS (`["hn","reddit","twitter","blog","web_search"]`, the canonical order), collectorHealthKey(c) → `"collector-health:<c>"` (Redis key), COLLECTOR_HEALTH_QUEUE_NAME = `"collector-health"` (the dedicated BullMQ queue, D-110), COLLECTOR_HEALTH_SCHEDULER_KEY = `"collector-health:default"` (repeatable-scheduler key for the auto-check cron), COLLECTOR_HEALTH_LEAD_MINUTES = 30 (auto-check fires this many minutes before pipelineTime)
