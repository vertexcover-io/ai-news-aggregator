...
- **Apify token (super-admin)**: `PUT /api/super/app-credentials/apify {"apiToken":"…"}` → validates + encrypts → upserts `app_credentials` row (`key="apify_api_token"`) → `{ok:true, configured:true, updatedAt}` (no token in response). `GET /api/super/app-credentials` returns status booleans only. `DELETE /api/super/app-credentials/apify` removes the row. All three require `requireSuperAdmin` — 401 unauthed / 403 wrong role.
